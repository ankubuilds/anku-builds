...
export function HeroTerminal() {
  ...
  return (
    <div className="relative w-full max-w-[520px]">
      {/* Ambient glow */}
      <div className="absolute -inset-8 -z-10 rounded-3xl bg-[#FDAD04]/5 blur-3xl" />

      {/* Terminal */}
      <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/70 shadow-2xl shadow-black/40 backdrop-blur-xl dark:border-white/10">
        {/* Terminal header */}
        ...
        <div className="p-5 sm:p-6">
          ...
          <div className="min-h-[92px]">
            ...
            >
              <span className="text-[#FDAD04]">{currentStep.command}</span>
              <span className="ml-1 inline-block h-4 w-1.5 translate-y-0.5 animate-pulse bg-[#FDAD04]" />
            </div>

            <div
              key={`output-${step}`}
              className="hero-terminal-output mt-2 font-mono text-xs leading-6 text-white/45"
            >
              {currentStep.output}
            </div>
          </div>
          ...
          {/* Progress */}
          <div className="mt-6">
            <div className="mb-2 flex items-center justify-between font-mono text-[10px] text-white/35">
              <span>BUILD PROCESS</span>
              <span>{Math.round(((step + 1) / buildSteps.length) * 100)}%</span>
            </div>
          ...
          {/* Footer */}
          <div className="mt-6 flex items-center justify-between border-t border-white/10 pt-4 font-mono text-[10px] text-white/30">
...
function Status({
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-white/[0.025] px-3 py-3">
      <div className="font-mono text-[9px] tracking-wider text-white/30">
        {label}
      </div>
